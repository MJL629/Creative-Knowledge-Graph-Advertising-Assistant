...
import { runInitialGraphPipeline } from "../../../../lib/agents/graph-pipeline";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const result = await runInitialGraphPipeline(body, controller.signal);
    return Response.json({ ok: true, result });
  } catch (error) {
    const message = error instanceof Error ? error.message : "未知错误";
    const status = message.includes("不能为空") || message.includes("不能") || message.includes("至少需要") || message === "请求体不能为空" ? 400 : 502;
    return Response.json({ ok: false, error: { code: "GRAPH_DIVERGENCE_FAILED", message } }, { status });
  } finally {
    clearTimeout(timer);
  }
}
