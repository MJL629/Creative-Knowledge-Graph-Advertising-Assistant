...
import { runRelationPipeline, type RelationRequest } from "../../../../lib/agents/graph-pipeline";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const result = await runRelationPipeline(body, controller.signal);
    return Response.json({ ok: true, result });
  } catch (error) {
    const message = error instanceof Error ? error.message : "未知错误";
    const isInputError = ["不能为空", "不能", "不能相同", "缺少"].some((word) => message.includes(word));
    return Response.json({ ok: false, error: { code: isInputError ? "RELATIONS_INPUT_INVALID" : "RELATIONS_FAILED", message } }, { status: isInputError ? 400 : 502 });
  } finally {
    clearTimeout(timer);
  }
}
